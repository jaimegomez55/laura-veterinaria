  ...
  return (
    <div>
      <div className="flex items-center gap-2 lg:gap-4">
        ...
        >
          <svg viewBox="0 0 24 24" className="w-5 h-5 lg:w-6 lg:h-6" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
        ...
        >
          {reviewsData.map((review, i) => (
            <div key={i} className="snap-start shrink-0 w-full md:w-[calc(50%-1rem)] lg:w-[calc(33.3333%-1.3334rem)]">
        ...
        <button
          type="button"
          onClick={() => scrollToIndex(Math.min(reviewsData.length - 1, activeIndex + 1))}
      ...
      <div className="flex justify-center items-center gap-2 mt-6">
        {reviewsData.map((_, i) => (
          <button
            ...
            className={`w-2.5 h-2.5 rounded-full transition-colors duration-300 ${i === activeIndex ? 'bg-[#1e3a5f]' : 'bg-gray-300 hover:bg-gray-400'}`}
          />
        ))}
      </div>
    </div>
