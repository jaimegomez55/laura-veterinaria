...
export default function ReferralForm() {
  ...
  const handleFiles = (files: File[]) => {
    ...
    for (const f of files) {
      if (newFiles.length >= MAX_FILES) { setUploadError(`Máximo ${MAX_FILES} archivos permitidos.`); break; }
      const ext = '.' + f.name.split('.').pop()?.toLowerCase();
      if (!ALLOWED_EXT.includes(ext)) { setUploadError(`"${f.name}" no es un formato permitido. Solo PDF, JPG o PNG.`); continue; }
      if (f.size > MAX_FILE_SIZE) { setUploadError(`"${f.name}" supera el límite de 10 MB.`); continue; }
  ...
  const validateForm = () => {
    ...
    const required = ['clinica', 'veterinario', 'email', 'tutor', 'tutor_telefono', 'nombre_mascota', 'especie', 'raza', 'peso', 'edad', 'motivo', 'rgpd'];
    const errors: Record<string, boolean> = {};
    let valid = true;
    required.forEach((field) => {
      const val = formData.get(field);
      if (!val || (typeof val === 'string' && val.trim() === '')) { errors[field] = true; valid = false; }
      if (field === 'email' && val && typeof val === 'string') {
        if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(val)) { errors[field] = true; valid = false; }
      }
    });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      for (const file of selectedFiles) {
        ...
        const cloudRes = await fetch(CLOUDINARY_UPLOAD_URL, { method: 'POST', body: cloudData });
        if (!cloudRes.ok) throw new Error(`Error subiendo "${file.name}"`);
  ...
  const inputClass = (field: string) =>
    `w-full bg-white border-2 rounded-xl px-4 py-4 outline-none transition-all font-['Merriweather'] text-lg text-[#1e3a5f] focus:border-[#1e3a5f] ${formErrors[field] ? 'border-red-500 ring-2 ring-red-100' : 'border-[#E8E4DD]'}`;
    ...
    <>
      ...
      <form ref={formRef} onSubmit={handleSubmit} className="space-y-10" noValidate>
        ...
        <div className="bg-white border-2 border-[#E8E4DD] rounded-3xl p-6 md:p-10 shadow-xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="flex flex-col gap-3">
              ...
              <input type="text" name="clinica" placeholder="Clínica Veterinaria..." className={inputClass('clinica')} />
            ...
            <div className="flex flex-col gap-3">
              ...
              <input type="text" name="veterinario" placeholder="Nombre y apellidos" className={inputClass('veterinario')} />
            ...
            <div className="flex flex-col gap-3">
              ...
              <input type="email" name="email" placeholder="correo@clinica.com" className={inputClass('email')} />
            ...
            <div className="flex flex-col gap-3">
              ...
              <input type="tel" name="telefono" placeholder="600 000 000" className="w-full bg-white border-2 border-[#E8E4DD] rounded-xl px-4 py-4 outline-none transition-all font-['Merriweather'] text-lg text-[#1e3a5f] focus:border-[#1e3a5f]" />
            </div>
          </div>
        </div>
        ...
        <div className="bg-white border-2 border-[#E8E4DD] rounded-3xl p-6 md:p-10 shadow-xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="flex flex-col gap-3">
              ...
              <input type="text" name="tutor" placeholder="Nombre completo" className={inputClass('tutor')} />
            ...
            <div className="flex flex-col gap-3">
              ...
              <input type="tel" name="tutor_telefono" placeholder="600 000 000" className={inputClass('tutor_telefono')} />
            ...
            <div className="flex flex-col gap-3 md:col-span-2">
              ...
              <input type="email" name="tutor_email" placeholder="tutor@email.com" className="w-full bg-white border-2 border-[#E8E4DD] rounded-xl px-4 py-4 outline-none transition-all font-['Merriweather'] text-lg text-[#1e3a5f] focus:border-[#1e3a5f]" />
            </div>
          </div>
        </div>
        ...
        <div className="bg-white border-2 border-[#E8E4DD] rounded-3xl p-6 md:p-10 shadow-xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="flex flex-col gap-3">
              ...
              <input type="text" name="nombre_mascota" placeholder="Nombre de la mascota" className={inputClass('nombre_mascota')} />
            ...
            <div className="flex flex-col gap-3">
              ...
              <select name="especie" className={inputClass('especie')}>
            ...
            <div className="flex flex-col gap-3">
              ...
              <input type="text" name="raza" placeholder="Raza" className={inputClass('raza')} />
            ...
            <div className="flex flex-col gap-3">
              ...
              <input type="number" name="peso" placeholder="Ej: 8.5" step="0.1" min="0" className={inputClass('peso')} />
            ...
            <div className="flex flex-col gap-3">
              ...
              <input type="text" name="edad" placeholder="Ej: 3 años" className={inputClass('edad')} />
            ...
            <div className="flex flex-col gap-3">
              ...
              <select name="sexo" className="w-full bg-white border-2 border-[#E8E4DD] rounded-xl px-4 py-4 outline-none cursor-pointer transition-all font-['Merriweather'] text-lg text-[#1e3a5f] focus:border-[#1e3a5f]">
                <option value="">Seleccionar...</option>
        ...
        <div className="bg-white border-2 border-[#E8E4DD] rounded-3xl p-6 md:p-10 shadow-xl">
          ...
          <div className="space-y-8">
            <div className="flex flex-col gap-3">
              ...
              <textarea name="motivo" rows={4} placeholder="¿Cómo puede ayudar la nutrición en este caso? Describe el motivo principal de la derivación..." className={inputClass('motivo')} />
            ...
            <div className="flex flex-col gap-3">
              ...
              <textarea name="pruebas" rows={3} placeholder="Analíticas, ecografías, radiografías u otras pruebas realizadas..." className="w-full bg-white border-2 border-[#E8E4DD] rounded-xl px-4 py-4 outline-none resize-none transition-all font-['Merriweather'] text-lg text-[#1e3a5f] focus:border-[#1e3a5f]" />
            </div>
            <div className="flex flex-col gap-3">
              ...
              <textarea name="tratamientos" rows={3} placeholder="Medicación actual, suplementos, dieta en curso..." className="w-full bg-white border-2 border-[#E8E4DD] rounded-xl px-4 py-4 outline-none resize-none transition-all font-['Merriweather'] text-lg text-[#1e3a5f] focus:border-[#1e3a5f]" />
            </div>
          </div>
        </div>
        ...
        <div className="bg-white border-2 border-[#E8E4DD] rounded-3xl p-6 md:p-10 shadow-xl">
          ...
          >
            <input ref={fileInputRef} type="file" multiple accept=".pdf,.jpg,.jpeg,.png" className="hidden" onChange={(e) => e.target.files && handleFiles(Array.from(e.target.files))} />
            <div className="text-5xl mb-6">📂</div>
          ...
          <div className="bg-[#F8F6F3] border-2 border-[#E8E4DD] rounded-3xl p-6 md:p-10">
            <div className="flex items-start gap-5">
              ...
              <label htmlFor="rgpd" className="text-lg text-[#1e3a5f] leading-relaxed cursor-pointer font-['Merriweather'] font-medium">
                He informado al propietario de que sus datos y los de su mascota serán compartidos con Laura Quintero para recibir asesoramiento nutricional. Acepto la{' '}
                <a href="/politica-de-privacidad" target="_blank" className="text-[#f2bac9] font-bold hover:underline decoration-2">política de privacidad</a>.
